...
# generated file type
    ...
    if [ ! -f "adguardhome_gfwlist.txt" ];then
            touch "adguardhome_gfwlist.txt"
    else
            :
    fi
    file_lines_old=`cat adguardhome_gfwlist.txt | wc -l`
    file_lines_new=`cat ${temporary_file} | wc -l`
    if [[ `expr $file_lines_new + 5 - $file_lines_old` -ge 0 ]]; then
        echo "# Generated by gfwlist2adguardhome at $(date '+%F %T')" >adguardhome_gfwlist.txt
        echo "tls://dot.pub" >>adguardhome_gfwlist.txt
        echo "https://doh.pub/dns-query" >>adguardhome_gfwlist.txt
        echo "tls://dns.alidns.com" >>adguardhome_gfwlist.txt
        echo "https://dns.alidns.com/dns-query" >>adguardhome_gfwlist.txt
        perl -pe "s@^.*+\$@[/$&/]$dns_addr@" ${temporary_file} >>adguardhome_gfwlist.txt
        echo "type: adguardhome_server    file: adguardhome_gfwlist.txt updated!"
    else
        echo "type: adguardhome_server    file: adguardhome_gfwlist.txt not updated!"
    fi
